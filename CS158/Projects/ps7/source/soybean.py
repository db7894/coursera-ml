...
def generate_output_codes(num_classes, code_type) :
    ...
    if code_type = 'ova':
        ...
        R = np.repeat(ones[:,np.newaxis], num_classifiers, axis=1) # when we do newaxis will be come vertical
...
class MulticlassSVM :
    ...
    def fit(self, X, y) :
        ...
        neg_ndx = []
        for x in X:
            idx = list(np.nonzero(self.predict(x) == C)[0]) # get indices where class equals C
