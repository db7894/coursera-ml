...
def apply_PCA_from_Eig(X, U, l, mu) :
    ...
    Ul = U[ : , -l: ] # keep all rows but only the last l columns since we want the largest l eigenvalues
    Z = (Ul.T).dot(X - mu)
    ### ========== TODO : END ========== ###
    return Z, Ul
...
def reconstruct_from_PCA(Z, U, mu) :
    ...
    X_rec = Z.dot(U.T) + mu # reconstruction = PC scores * Eigenvectors.T + Mean
    ### ========== TODO : END ========== ###
    return X_rec